...
class Table(Node):
    rows: list[
        list[list[Text]]
    ]  # list of rows, each row is list of cells, each cell is list of Text spans
...
def _flatten_inline(
    tokens: list[dict], bold=False, italic=False, strikethrough=False, url=None
) -> list[Text]:
...
def _extract_list_items(list_tok: dict, depth: int = 0) -> list[ListItem]:
    ...
    for item_tok in list_tok.get("children", []):
        ...
        if inline_children:
            items.append(
                ListItem(children=inline_children, ordered=ordered, depth=depth)
            )
    return items
...
def generate_requests(
    nodes: list[Node], tab_id: str | None = None
) -> tuple[str, list[dict]]:
        ...
        if isinstance(node, Heading):
            ...
            if node.ordered:
                format_actions.append(
                    (list_start, list_end - 1, "ordered_list", node.depth)
                )
            else:
                format_actions.append(
                    (list_start, list_end - 1, "unordered_list", node.depth)
                )
            ...
            table_text = (
                "\n".join(
                    "\t".join(_cell_plain(cell) for cell in row) for row in node.rows
                )
                + "\n"
            )
            ...
            format_actions.append(
                (table_start, table_end - 1, "table", (num_rows, num_cols, node.rows))
            )
...
def markdown_to_requests(
    md: str, tab_id: str | None = None
) -> tuple[list[dict], list[list[list[list[Text]]]], list[PushWarning]]:
